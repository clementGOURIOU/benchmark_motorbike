...
Pressure: 
*  FOAM-DIC-PCG: is an iterative solver provided by OpenFOAM that combines a Diagonal-based Incomplete-Cholesky (DIC) preconditioner with a Conjugate Gradient (PCG) solver. DIC is a common preconditioner in OpenFOAM for its easy configuration and high efficiency 
...
*  PETSc-AMG-CG(*):  is  the  PETSc  counterpart  of  the  FOAM-GAMG-PCG  method.  In this  solver  we  use  an  algebraic  multigrid  method,  named  BoomerAMG,  provided  by the third-party library of PETSc named Hypre. 
...
The following Table summirizes the different Preconditioner/solver pairs used to solve the pressure equation 
